...
| 目标 | 推荐路径 | 关注重点 |
| --- | --- | --- |
| 建立基础概念 | 语言模型演化 -> [Transformer 模型](./transformer.md) -> [预训练范式](../training/pre-training.md) | token、语言建模、注意力、训练目标 |
| 理解现代 LLM 架构 | [Transformer 模型](./transformer.md) -> 高效架构 -> [混合专家模型](./mixture-of-experts.md) | Decoder-only、KV Cache、GQA、MoE |
| 理解能力来源 | [预训练范式](../training/pre-training.md) -> [扩展定律](./scaling-law.md) -> [后训练](../training/post-training.md) | 数据、算力、偏好优化、评测 |
| 研究推理模型 | [后训练](../training/post-training.md) -> [推理能力](./reasoning.md) -> [扩展定律](./scaling-law.md) | RLVR、GRPO、verifier、test-time scaling |
...
| 架构层 | attention、FFN、normalization、MoE | 如何组织参数和计算路径 | [Transformer 模型](./transformer.md)、[混合专家模型](./mixture-of-experts.md) |
| 训练层 | pre-training、continued pre-training | 能力如何从数据和算力中形成 | [预训练范式](../training/pre-training.md)、[扩展定律](./scaling-law.md) |
| 对齐层 | SFT、RLHF、DPO、GRPO | 模型如何变得可用、可控、可交互 | [后训练](../training/post-training.md) |
...
| 评测层 | benchmark、human eval、contamination check | 如何判断能力是否真实可靠 | [预训练范式](../training/pre-training.md)、[后训练](../training/post-training.md) |
