...
SFT 的意义在于建立稳定的行为接口，而非向模型灌入所有知识。高质量 SFT 让后续 [强化学习](./rl.md) 有一个可用的初始策略，也让 [蒸馏](./distillation.md) 的学生具备基本生成能力；如果模板、掩码或示范本身错误，后续优化通常只会放大这些问题。
...
1. 少而精的数据比多而烂的数据重要得多。
2. 避免灾难性遗忘的方法是混合一定比例（例如 5% ~ 10%）的通用数据
3. loss 计算时要 mask 掉：`system_prompt`、`user_prompt` 和 `tool_observation`，只有 `assistant reasoning content`、`assistant content` 和 `tool_call` 才进入 loss 计算。
4. chat template 要保持训推一致。
