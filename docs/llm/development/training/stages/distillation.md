...
教师读取「prompt + 学生回答」并在每个位置评价下一 token 分布，不会重新生成另一条答案。只有这样，$P_T$ 与 $P_S$ 才对应同一组状态。
...
蒸馏成功的判据是学生在目标质量约束内取得可验证的部署或能力整合收益，仅凭 loss 足够低不能下结论。若模型没有变小、吞吐没有提升，且能力也未整合，蒸馏链路即使收敛也没有完成工程目标。
