...
把 Transformer 先理解为重复堆叠的“注意力 -> 前馈网络”模块：输入 token 经 [Tokenizer](./tokenizer.md) 和 embedding 转成向量，再通过带残差连接与归一化的层更新表示。实现或排错时，先用极短序列验证张量形状、因果 mask 和下一个 token 标签对齐。

## 简单案例

对序列“我 喜欢 AI”进行 Decoder-only 前向计算时，第 2 个位置只能注意到“我”和“喜欢”，不能注意未来的“AI”。打印注意力 mask 并检查最后一个位置的 logits 是否用于预测下一个 token；若 mask 方向或标签右移错误，训练 loss 可能下降却会发生答案泄漏。
...
再经过下投影回到隐藏维度。FFN 通常占据 Transformer block 的大量参数，因此 [MoE](./moe.md) 常用多个专家 FFN 替换 dense FFN。
...
三类结构对应的语言建模目标和训练路线详见 [预训练](../training/stages/pre-training.md)。
...
它不是单一技术，而是一组围绕 Transformer 的改造：位置编码、注意力计算、KV Cache、稀疏注意力、分组查询、投机解码等。本文只讨论模型结构和算法动机，部署框架放在 [模型推理与部署](../../serving/index.md) 章节中。
...
高效架构的价值在于支撑更大的模型、更长的上下文和更低的推理成本。它与 [扩展定律](./scaling-law.md) 和 [MoE](./moe.md) 共同构成现代大模型规模化的工程基础。
