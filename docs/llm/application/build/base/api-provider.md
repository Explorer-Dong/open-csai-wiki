...
先用官方平台和最小额度验证目标模型的 API、流式输出、限速、错误码和数据政策，再用真实输入长度压测并发、延迟与成本。确需聚合多家模型时选择有明确主体、账单和隐私条款的服务，并保留迁回官方端点的能力。

例如代码任务不能只比较每百万 Token 单价：应使用同一批仓库任务，记录成功率、输入输出 Token、重试次数与 P95 延迟，再计算每个成功任务的总成本。密钥只放在环境变量或密钥系统中，不写入代码和截图。
