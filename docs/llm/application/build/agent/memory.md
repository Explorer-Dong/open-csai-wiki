...
先用结构化状态保存当前目标、已完成步骤和关键结果，不要直接把全部历史对话当作记忆。确有跨会话需求时，再将经过筛选的事实写入长期存储，并在读取时执行权限过滤、时效检查和来源追踪。
...
客服 Agent 将“用户偏好中文回答”作为可更新的长期记忆，并在新会话开始时按用户 ID 检索。订单金额和地址这类可能过期或敏感的信息不直接写入通用记忆，而是在有权限时从业务系统实时读取；这样可避免过期记忆被当作事实复述。
