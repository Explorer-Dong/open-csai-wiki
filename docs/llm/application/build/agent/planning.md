...
将任务拆成具有明确输出和依赖关系的步骤，每完成一步就依据实际结果更新计划。简单任务直接执行；只有涉及多步依赖或不确定环境时才引入规划，并设置最大步骤数与失败终止条件。
...
代码修复任务先拆为“复现测试失败 -> 定位相关文件 -> 进行最小修改 -> 运行测试 -> 汇报 diff”。每一步都根据真实工具输出更新计划；若测试无法复现，Agent 应停止猜测并请求更多信息。
