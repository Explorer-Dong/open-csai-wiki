...
先阅读 v1 的最小 Agent 循环，确认 Messages API、工具定义、执行与结果回注能闭环；随后按 v2 至 v8 的顺序加入 Hook、Skill、子 Agent、上下文压缩、持久记忆、任务系统和隔离。每次演进都先运行对应版本的案例，验证新增模块没有改变既有工具行为。

若只为理解架构，可先看下方版本对比和五层架构总结；若要运行项目，应以链接仓库中的依赖与 README 为准，并在无密钥、受限目录中测试工具调用。
