...
只有任务可以独立分解且并行收益大于协调成本时，才使用多 Agent。为每个 Agent 定义输入、输出、资源范围和完成条件，由一个协调者合并结果并处理冲突；共享写入操作必须串行化或采用明确的所有权规则。
...
研究任务由检索 Agent 收集来源、分析 Agent 归纳证据、审校 Agent 检查引用。协调器只接受带来源的结论，并在冲突时要求回到原始证据，避免多个 Agent 相互重复或放大错误。
