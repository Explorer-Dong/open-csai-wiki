...
先选择一个只读 MCP Server，在客户端配置其启动命令或远程地址，确认能够列出 Tool、Resource 和 Prompt，再调用一个无副作用工具。生产接入还需固定协议与 Server 版本、限制可见能力、校验参数并记录调用日志。

MCP 标准化的是 Host 与外部能力之间的发现和调用协议，不会自动赋予模型权限，也不会替代 Function Calling 的模型侧工具选择。下面先从多对多集成问题讲起，再展开协议角色与 FastMCP 案例。
